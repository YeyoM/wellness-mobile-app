import { MILLISECONDS_IN_A_WEEK } from "../dateToMilliseconds";

export default function retreatOneWeek(zoomState, setZoomState, minDomain) {
  if (!zoomState) {
  ...
  if (!minDomain || typeof minDomain !== "number") {
    throw new Error("No minDomain provided");
  }

  if (typeof zoomState.x[0] === "number") {
    const left_prevUnixTime = zoomState.x[0];
    const left_nextUnixTime = left_prevUnixTime - MILLISECONDS_IN_A_WEEK;
    const left_newDate = new Date(left_nextUnixTime);

    const right_prevUnixTime = zoomState.x[1];
    const right_nextUnixTime = right_prevUnixTime - MILLISECONDS_IN_A_WEEK;
    const right_newDate = new Date(right_nextUnixTime);

    if (left_nextUnixTime < minDomain) {
      return;
    }
