...
import { collection, addDoc, setDoc, doc, runTransaction, getDoc } from "firebase/firestore";
...
export default function SelectNumberDaysPerWeek({ navigation }) {
  ...
  const [ routine_, setRoutine_ ] = useState(null);
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // 3. save the days to firebase with auto-generated ids on the days collection and save the ids to the daysIds array
      const daysRef = await Promise.all(days_.map(async (day) => {
      ...
      // 4. update the routine with the daysIds array
      await setDoc(doc(FIRESTORE, "routines", routineRef.id), { days: daysIds }, { merge: true });
  ...
  return (
    <View style={styles.container}>
      <TopNavigationBar navigation={navigation} actualScreen={'Routine Creator'} back={!loading} />
      { error && <ErrorNotification message={'Error creating routine'} /> }
      { success && <SuccessNotification message={'Routine created, redirecting...'} /> }
      <View style={styles.content}>
        ...
        <Text style={{ color: '#fff', fontSize: 140, marginTop: 20, textAlign: 'center', fontWeight: 'bold' }}>{numberOfDays}x</Text>
        <View style={{ width: '90%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', marginTop: 20, backgroundColor: "#24262B", padding: 4, borderRadius: 16 }}>
          <Pressable onPress={() => setNumberOfDays(1)} style={numberOfDays === 1 ? styles.btnSelected : styles.btn}>
            <Text style={styles.btnText}>1</Text>
          </Pressable>
          <Pressable onPress={() => setNumberOfDays(2)} style={numberOfDays === 2 ? styles.btnSelected : styles.btn}>
            <Text style={styles.btnText}>2</Text>
          </Pressable>
          <Pressable onPress={() => setNumberOfDays(3)} style={numberOfDays === 3 ? styles.btnSelected : styles.btn}>
            <Text style={styles.btnText}>3</Text>
          </Pressable>
          <Pressable onPress={() => setNumberOfDays(4)} style={numberOfDays === 4 ? styles.btnSelected : styles.btn}>
            <Text style={styles.btnText}>4</Text>
          </Pressable>
          <Pressable onPress={() => setNumberOfDays(5)} style={numberOfDays === 5 ? styles.btnSelected : styles.btn}>
            <Text style={styles.btnText}>5</Text>
          </Pressable>
        </View>
        <Text style={{ color: '#fff', fontSize: 16, marginVertical: 20, textAlign: 'center' }}>I’m commited to exercising {numberOfDays}x weekly</Text>
        {
          loading 
