...
// REPLACE WITH A CUSTOM FUNCTION WHERE THE WORKOUTS ARE FETCHED FROM THE DATABASE
...
export default function SharedProfileStats({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getSpecificWorkouts(workoutsIds)
      .then((workouts) => {
        setWorkouts(workouts);
        const stats = getSharedStatsData(workouts, profile);
        const {
          weightProgressData,
          ...
          weightRecord: stats.weightRecord,
        });
        ...
        console.log("stats.weightLiftedRecord", stats.weightLiftedRecord);
        console.log("weightLiftedProgressData", weightLiftedProgressData);
        console.log(
          "weightLiftedProgressDataByWeek",
          weightLiftedProgressDataByWeek,
        );
        ...
        >
          ...
          <View style={styles.graphContainer}>
            ...
            <View style={{ overflow: "hidden", width: "100%" }}>
              {!loading && selectedCategory === "Weight Lifted" ? (
                weightLiftedLineData && weightLiftedLineData.length > 0 ? (
                  <LineChart
                    hideDataPoints
                    isAnimated
              ...
              ) : !loading && selectedCategory === "User Weight" ? (
                weightLineData && weightLineData.length > 0 ? (
                  <LineChart
                    hideDataPoints
                    isAnimated
                    animationDuration={1200}
                ...
                caloriesLineData && caloriesLineData.length > 0 ? (
                  <LineChart
                    hideDataPoints
                    isAnimated
                    animationDuration={1200}
          ...
          <View style={styles.repMaxesHeader}>
            <Text style={styles.textHeader}>One rep maxes</Text>
          </View>
          <CarouselRepsMaxes exercises={[]} />
        </View>
      </ScrollView>
    </View>
  );
